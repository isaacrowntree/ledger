...
def parse_loan(monkeypatch):
    def _parse(rows):
        ...
        return BankwestLoanPDFParser().parse_statement(Path("homeloan.pdf")).rows
    return _parse
...
def parse_offset(monkeypatch):
    def _parse(rows):
        monkeypatch.setattr(bankwest_account_pdf, "extract_rows", lambda *a, **k: rows)
        return BankwestOffsetPDFParser().parse_statement(Path("offset.pdf")).rows
    return _parse
...
class TestLoanSigns:
    def test_debit_balances_are_negative_money_owed(self, parse_loan):
        ...
        assert txns[0].raw["balance"] == "-301500.00"
...
class TestOffsetSigns:
    def test_fee_is_negative_and_deposit_positive(self, parse_offset):
        ...
        assert BankwestOffsetPDFParser.source_type == "bankwest-offset"
...
class TestMetadata:
    def test_records_account_and_period(self, parse_loan):
        ...
        assert BankwestLoanPDFParser.source_type == "bankwest-loan"
