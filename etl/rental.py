...
Reads rental_properties from tax.yaml and computes the taxpayer's
deductible share of expenses (and assessable share of income) using
per-rule apply modes:
    [ownership, area, time]
...
def compute_rental_deductions(
    ...
    """For each rental property, compute the taxpayer's share of income +
    per-ATO-label deductions.

    Returns:
      {
        "<property_key>": {
            "income": {"gross": 0.0, "taxpayer_share": 0.0},
            "deductions": [
                {"ato_label": "Body corporate fees", "gross": 0.0,
                 "factor": 0.5, "taxpayer_share": 0.0, "n_txns": 0},
                ...
            ],
    ...
    for rental in tax_config.get("rental_properties", []):
        ...
        for mapping in rental.get("expense_mapping", []):
            ...
            gross_abs = abs(gross)
            taxpayer_share = round(gross_abs * factor, 2)
            total_taxpayer_deductions += taxpayer_share
        ...
        if depreciation:
            total_taxpayer_deductions += depreciation
            deductions.append({
                "ato_label": "Depreciation (capital allowances)",
                "gross": round(depreciation, 2),
                "factor": 1.0,
                "apply": ["depreciation"],
                "taxpayer_share": round(depreciation, 2),
                "n_txns": 0,
            })
        ...
        results[name] = {
            "income": {
                ...
                "taxpayer_share": round(income_gross * ownership_f, 2),
            },
            "deductions": deductions,
            "total_deductions": round(total_taxpayer_deductions, 2),
            "net": round(income_gross * ownership_f - total_taxpayer_deductions, 2),
        }
...
def format_summary(results: dict) -> str:
    ...
    for name, data in results.items():
        ...
        out.append(f"    × ownership ({inc['factor']:.2%}) → your share: ${inc['taxpayer_share']:>10,.2f}")
        out.append(f"\n  Deductions:")
        out.append(f"  {'ATO label':<35} {'gross':>11} {'factor':>8} {'apply':<25} {'your share':>11}")
        for d in data["deductions"]:
            apply_str = "+".join(d["apply"]) if d["apply"] else "100%"
            out.append(
                f"  {d['ato_label']:<35} {d['gross']:>11,.2f} {d['factor']:>8.4f} "
                f"{apply_str:<25} {d['taxpayer_share']:>11,.2f}"
            )
        out.append(f"\n  Total deductions: ${data['total_deductions']:>12,.2f}")
